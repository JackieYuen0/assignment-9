...
# Your code here
phonex_promo = sales_df.loc[(sales_df['Product'] == "PhoneX") & (sales_df['Promotion'] == 'Yes')]
print(phonex_promo)
...
# Your code here
feb_sales = sales_df.loc[sales_df['Date'].str[0:7] == "2024-02"]
print(feb_sales)
...
for product in sales_df["Product"]:
    temp_df = sales_df[sales_df['Product'] == product]
    total_sales = 0
    for sales in temp_df['Total_Sales']:
        total_sales += sales
    if total_sales > best_product_sales:
...
# Your code here
promo_units_sold = sales_df.groupby('Promotion')['Total_Sales'].mean()
promo_revenue = sales_df.groupby('Promotion')['Total_Sales'].sum()
print(promo_units_sold, promo_revenue)
promo_comparison = {
    'promo_avg_sales': promo_units_sold['Yes'],
    'no_promo_avg_sales': promo_units_sold['No'],
    'promo_total_revenue': promo_revenue['Yes'],
    'no_promo_total_revenue': promo_revenue['No']
    }
...
# Your code here
missing_percentages = 100*(missing_counts/len(sales_df.index))
print(missing_percentages , 'percentages')
